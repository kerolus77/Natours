...
export const requireBookingTour = catchAsync(async (req, res, next) => {
    const booking = await Booking.findOne({ tour: req.body.tour, user: req.user.id });
    if (!booking)
        return next(new AppError('You can only review tours you booked', 403));
    next();
});
export const restrictReviewOwner = catchAsync(async (req, res, next) => {
