...
const releaseExpiredHolds=async(session:mongoose.ClientSession)=>{
    ...
    for(const hold of expiredHolds){
        await Tour.updateOne(
            {_id:hold.tour,'startDates.startDate':hold.startDate},
            {$inc:{'startDates.$.participants':-1},$set:{'startDates.$.soldOut':false}},
            {session}
        );
        await Booking.updateOne(
            {_id:hold._id,status:'pending'},
            {$set:{status:'expired',paid:false},$unset:{expiresAt:1}},
            {session}
        );
    }
};
...
const reserveSeat=async(
    ...
    if(existingBooking) throw new AppError('You already have a booking for this tour date',400);

    const updatedTour=await Tour.findOneAndUpdate(
        {_id:tourId,startDates:{$elemMatch:{
    ...
    if(!updatedTour) throw new AppError('Tour is sold out for this date',400);
    ...
    if(!hold) throw new AppError('Could not create booking hold',500);
    return hold;
};

const releaseHold=async(bookingId:mongoose.Types.ObjectId)=>{
    const session=await mongoose.startSession();
    try{
        await session.withTransaction(async()=>{
            const hold=await Booking.findOneAndUpdate(
...
if (!tour) return next(new AppError('No tour found with that ID',404));
    ...
    const startDate=tour.startDates.find(
        date=>date.startDate.getTime()===selectedDate.getTime()
    );
    if (!startDate) return next(new AppError('Start date not found',404));
    ...
    let hold;
    try{
        hold=await dbSession.withTransaction(() =>
            reserveSeat(tour.id,req.user.id,selectedDate,dbSession)
        );
    }finally{
        await dbSession.endSession();
    }
    if(!hold) return next(new AppError('Could not create booking hold',500));
...
export const createBookingCheckout=catchAsync(async(req:Request,res:Response,next:NextFunction)=>{
    ...
    if (!bookingId) return next(new AppError('Invalid booking details',400));
    ...
    try {
        await dbSession.withTransaction(async()=>{
            ...
            const booking=await Booking.findOneAndUpdate(
                {_id:bookingId,status:'pending',expiresAt:{$gt:new Date()}},
                {$set:{
                    paid:true,
                    status:'paid',
            ...
            if(!booking) throw new AppError('This booking hold has expired',400);
        });
    } finally {
        await dbSession.endSession();
    }
