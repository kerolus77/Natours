...
type ReviewQuery = Query<unknown, IReview> & {
  r?: Query<IReview | null, IReview>;
};
...
reviewSchema.pre(/^findOneAnd/,function(this:ReviewQuery){
    this.r=this.findOne();
})
reviewSchema.post(/^findOneAnd/,async function(this:ReviewQuery){
    const review=await (this as ReviewQuery).r;
    if(!review)return;
